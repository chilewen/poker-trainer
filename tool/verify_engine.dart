...
void main() {
  ...
  // 阈值按真人频率定：顶对在这条线上该有明显价值加注（六成上下），但
  // 「没位置一律加得比有位置狠」是错的——加完还要在不利位置打后面两条街。
  // 以前这里卡 cr > 0.7 / 领先 20 个点，正好把那个夸张的倍率锁死了
  // （实测顶对没位置 82% 加注），现在改成「方向对 + 有明显差距」。
  final cr = checkRaiseRates();
  check(cr.cr > 0.5,
      '过牌-加注：先过牌再面对下注，顶对会用加注回收价值 '
      '(${(100 * cr.cr).toStringAsFixed(0)}%)');
  check(cr.cr > cr.base + 0.05,
      '过牌-加注比「直接面对下注」更凶 '
