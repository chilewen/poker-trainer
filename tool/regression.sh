...
# flutter test——后者只按**文件**并行，engine_test.dart 那 67 条用例它拆不开。
...
#   --probes          额外跑 5 个诊断探针（ai_probe / ai_sim 那一族）。它们只打印
#                     比例、不会让回归失败，却占掉整套里最大一块 CPU（约 46s），
...
# verify_engine 会失败）。它们加起来约 46 秒 CPU，是全套里最贵的一块，所以
...
if (( PROBES )); then
  ...
  # ai_probe 是全套里最重的一个（68 格 × 300 手），拆 4 片并行：每格只由一个
